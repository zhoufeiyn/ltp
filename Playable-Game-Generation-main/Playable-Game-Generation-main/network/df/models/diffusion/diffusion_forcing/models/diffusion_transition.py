...
class DiffusionTransitionModel(nn.Module):
    def __init__(self, x_shape, z_shape, external_cond_dim, cfg, config):
        ...
        self.num_gru_layers = cfg.num_gru_layers # 0
        self.num_mlp_layers = cfg.num_mlp_layers # 0
        self.timesteps = cfg.timesteps # 1000
        self.sampling_timesteps = cfg.sampling_timesteps # 4
        self.beta_schedule = cfg.beta_schedule # sigmoid
        self.objective = cfg.objective # pred_v  
        self.use_snr = cfg.use_snr # true
        self.use_cum_snr = cfg.use_cum_snr
        self.snr_clip = cfg.snr_clip # 5.0
        self.cum_snr_decay = cfg.cum_snr_decay # 0.4096000000000001
        self.ddim_sampling_eta = cfg.ddim_sampling_eta # 0.0
        self.clip_noise = cfg.clip_noise # 6
        self.p2_loss_weight_gamma = cfg.p2_loss_weight_gamma # 0
        self.p2_loss_weight_k = cfg.p2_loss_weight_k
        self.schedule_fn_kwargs = cfg.schedule_fn_kwargs # {}
        self.self_condition = cfg.self_condition # false
        self.network_size = cfg.network_size # 48
        self.return_all_timesteps = cfg.return_all_timesteps # false    
    ...
    def _build_model(self):
        ...
        if len(self.x_shape) != 1:
            if self.config.use_dit: # true
                self.model = TransitionDiT(
                    dim=self.x_shape[-1], # 32
                    z_channel=z_channel, # 32
                    x_channel=x_channel, # 4
                    external_cond_dim=self.external_cond_dim, # 1
                    num_gru_layers=self.num_gru_layers, # 0
                    self_condition=self.self_condition,
                    model_name=self.config.dit_name, # dit_small
    ...
    def _build_buffer(self):
        ...
        betas = beta_schedule_fn(self.timesteps, **self.schedule_fn_kwargs)# 每个步长对应加多少噪声
    ...
    def forward(
        ...
        """
        :param z: z at current time step (batch_size,32,32,32)
        :param x_next: ground truth x x at next step (batch_size,c,h,w)
        :param external_cond: external_cond to be conditioned on (batch_size,action_dim)
        ...
        noise = torch.randn_like(x_next) # (batch_size,c,h,w)
        noise = torch.clamp(noise, -self.clip_noise, self.clip_noise)
        noised_x_next = self.q_sample(x_start=x_next, t=t, noise=noise) # (batch_size,c,h,w)
    ...
    def model_predictions(self, x, t, z_cond, external_cond=None, x_self_cond=None):
        ...
        model_output = self.x_from_z(z_next) #（batch_size,4,32,32）
