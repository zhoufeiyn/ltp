...
class TransitionDiT(DiT):
    def __init__(
        ...
        super().__init__(
            ...
            z_cond_dim=z_channel, # 32
            self_condition=self_condition, # False
            config=config,
        )
        self.z_channel = z_channel
        self.x_channel = x_channel
    ...
    def forward(self, x, t, z_cond, external_cond=None, x_self_cond=None):
        # x: (batch_size,4,32,32), t: (batch_size,), z_cond: (batch_size,32,32,32), external_cond: (batch_size,action_dim), x_self_cond: (batch_size,4,32,32)
        z_next = super().forward(x, t, z_cond, external_cond, x_self_cond)
        if self.num_gru_layers:
            z_next = self.gru(z_next, z_cond)
