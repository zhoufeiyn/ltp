...
class DiffusionForcingBase(nn.Module):
    def __init__(self,config=None,device='cuda:0'):
        ...
        self.config = config
        use_mc_config = self.config.use_mc # False
        # cfg.context_frames = 16
        cfg.z_shape = [self.config.zeta, cfg.z_shape[1], cfg.z_shape[2]] # (32,32,32)
        if use_mc_config:
        ...
        # cfg.z_shape = [32, 64, 64]
        # cfg.diffusion.network_size = 64
        cfg.context_frames = self.config.context_len # 1
        self.cfg = cfg
        self.device = device
        self.x_shape = cfg.x_shape # (4,32,32)
        self.z_shape = cfg.z_shape # (32,32,32)
        self.frame_stack = cfg.frame_stack # 1
        self.cfg.diffusion.cum_snr_decay = self.cfg.diffusion.cum_snr_decay**self.frame_stack # 信噪比snr 0.4096000000000001
        self.x_stacked_shape = list(cfg.x_shape) # (4,32,32) ->[4,32,32]
        self.x_stacked_shape[0] *= cfg.frame_stack # 4*1=4
        ...
        self.context_frames = cfg.context_frames  # 4 number of context frames at validation time
        self.chunk_size = cfg.chunk_size # 1
    ...
    def _preprocess_batch(self, batch):
        ...
        # 假设 nonterminals = [[True, True], [True, False], [False, True]]
        # cumprod 结果：
        # masks[0] = [True, True]     # 第0步：都未终止
        # masks[1] = [True, False]    # 第1步：第1个序列继续，第2个序列终止
        # masks[2] = [False, False]   # 第2步：第1个序列也终止了，第2个序列保持终止
        nonterminals = nonterminals.bool().permute(1, 0) # (batch_size, n_frames) -> (n_frames, batch_size)
        ...
        if self.external_cond_dim:
            conditions = batch[1] # 获取动作数据 (batch_size, n_frames, action_dim)
            # 如单个batch: [[20], [20], [20], [20]]  (batch_size=1, n_frames=4, action_dim=1)
            
            conditions = torch.cat([torch.zeros_like(conditions[:, :1])+45, conditions[:, 0:-1]], 1)
            # 将第一个action加上45（初始帧，不动作），然后concat后面的action, 最后一帧的action去掉，则当前的img对应的是上一帧的action
            conditions = rearrange(conditions, "b (t fs) d -> t b (fs d)", fs=self.frame_stack).contiguous()
                #  比如 conditions = [
                #     [[20], [10]],  # 时间步0: batch0=20, batch1=10
                #     [[20], [10]],  # 时间步1: batch0=20, batch1=10
                #     [[20], [10]],  # 时间步2: batch0=20, batch1=10
                #     [[20], [10]]   # 时间步3: batch0=20, batch1=10
                # ]
                # # 原始: (batch_size, n_frames, action_dim) - 批次优先，变换后形状: (n_frames=4, batch_size=2, fs*action_dim=1)
        else:
            conditions = [None for _ in range(n_frames)]

        xs = self._normalize_x(xs)
        xs = rearrange(xs, "b (t fs) c ... -> t b (fs c) ...", fs=self.frame_stack).contiguous() 
        # (b,frames,c,h,w) -> (frames, batch_size, c,h,w), fs=1

        if self.learnable_init_z:
            init_z = self.init_z[None].expand(batch_size, *self.z_shape) 
            #初始化 z (1,32,32,32) -> (batch_size,32,32,32)
        else:
    ...
    def training_step(self, batch):
        ...
        for t in range(0, n_frames):
            ...
            if random() <= self.gt_cond_prob or (t == 0 and random() <= self.gt_first_frame):
                deterministic_t = 0
            # z （batch_size,32,32,32）,xs[t]:[batch_size,c,h,w], conditions[t]:[batch_size,action_dim]
