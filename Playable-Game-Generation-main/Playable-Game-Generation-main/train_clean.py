...
def sdxl_vae_encode(batch_data_images, vae_model, device, scale_factor=0.18215):
...
def train():
    ...
    diffusion_model = Flow_Matching_Model().to(device_obj)
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, total_samples, batch_size * num_frames):
            ...
            batch_data[0] = sdxl_vae_encode(batch_data[0], vae, device_obj)
            # 训练步骤
            try:
        ...
        if (epoch + 1) % gif_save_epoch == 0:
            ...
            diffusion_model.sample(vae, device=device_obj, scale_factor = 0.18215, shape = (1, 4, 32, 32), steps = 50, epoch = epoch+1)
...
class UNet(nn.Module):
    def __init__(self, in_ch=4, base_ch=64, time_dim=256):
        super().__init__()
...
class Flow_Matching_Model(nn.Module):
    ...
    @torch.no_grad()
    def sample(self, vae, device, scale_factor=0.18215, shape=(1, 4, 32, 32), steps=50, epoch=0):
        ...
        for k in range(steps):
            ...
            v = self.model(x, t)  # dx/dt = v_theta
            x = x + dt * v

        obs = vae.decode(x / scale_factor)
        obs = torch.clamp(obs * 0.5 + 0.5, min=0, max=1) * 255.0
        img = obs[0].permute(1, 2, 0).cpu().numpy().astype(np.uint8)
        img_pil = Image.fromarray(img)
        if not os.path.exists("output_overfit"):
            os.makedirs("output_overfit")
        img_pil.save(f'output_overfit/{epoch}.png')
